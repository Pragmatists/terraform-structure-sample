locals {
  brand                    = "pl-brand-two"
  environment              = "prod"
  environment_name         = "pl-brand-two-prod"
  web_address_client_panel = "client-panel.brand-two.example.pl"
}
...
module "client_panel_bucket" {
  ...
  organization = "${var.organization}"
  bucket_name  = "pl-brand-two-client-panel-prod"
}

resource "aws_route53_zone" "default" {
  name = "${local.web_address_client_panel}"
